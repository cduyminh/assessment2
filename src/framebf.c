...
void drawLineARGB32(int x, int y, int x1, int y1, unsigned int attr, unsigned int attr1)
{
    for (int i = 0; i <= x; i++)
    {
        drawPixelARGB32(i, y, attr1);

        for (int j = 0; j <= y; j++)
        {
            drawPixelARGB32(x, j, attr1);
        }
    }
    for (int i = 0; i <= x1; i++)
    {
        drawPixelARGB32(i, y1, attr1);

        for (int j = 0; j <= y1; j++)
        {
            drawPixelARGB32(x1, j, attr1);
        }
    }
...
// void drawCircleARGB32(int x, int y, int radius, unsigned int attr, int fill)
// {
//     for (int y = y1; y <= y2; y++)
//         for (int x = x1; x <= x2; x++)
//         {
