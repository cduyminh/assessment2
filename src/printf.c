...
void printf(const char *format, ...)
{
	...
	vcprintf((vcprintf_callback_t)uart_sendc, NULL, format, args);
	va_end(args);
}
			  ...
			  va_list args)
{
	...
	repeat:
		...
		case 'u':
			...
			{
			case 1:
				num = (uint8_t)va_arg(args, int);
				break;
			case 2:
				num = (uint16_t)va_arg(args, int);
				break;
			case 4:
				num = (uint32_t)va_arg(args, int);
				break;
			case 8:
				num = (uint64_t)va_arg(args, uint64_t);
				break;
			}
			if (precision == -1)
				precision = 0;
